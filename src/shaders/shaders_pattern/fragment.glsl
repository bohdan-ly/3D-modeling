...
void main() {
    ...
    // Pattern 17
    // float strenght = min(abs(vUv.y - 0.5), abs(vUv.x - 0.5));

    // Pattern 18
    // float strenght = max(abs(vUv.y - 0.5), abs(vUv.x - 0.5));

    // Pattern 19
    // float strenght = step(0.9, abs(vUv.y - 0.5) * 5.0);
    // strenght += step(0.9, abs(vUv.x - 0.5) * 5.0);

    // float strenght = step(0.2, max(abs(vUv.y - 0.5), abs(vUv.x - 0.5)));
    ...
    // float strenght = strenghtY * strenghtX;
    ...
    // Patternt 25
    // vec2 gridUv = vec2(floor(vUv.x * 10.0) / 10.0, floor((vUv.y + vUv.x) * 5.0) / 10.0);
    // float strenght = random(gridUv);
    ...
    // vec2 lightUvY = vec2(vUv.x, vUv.y * 0.1 + 0.45);
    ...
    // vec2 lightUvY = vec2(rotatedUv.x, rotatedUv.y * 0.1 + 0.45);
    // float lightX = 0.015 / distance(lightUvX, vec2(0.5));
    // float lightY = 0.015 / distance(lightUvY, vec2(0.5));
    // float strenght = lightX * lightY;
