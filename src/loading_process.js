 ...
 * Loaders
 */
const loadingBar = document.querySelector('div.loading-bar');
const loadingManager = new THREE.LoadingManager(
  ...
  (itemUrl, itemLoaded, itemsTotal) => {
    const progress = Math.round((itemLoaded / itemsTotal) * 100) / 100;
...
const overlayMaterial = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    void main() {
      gl_FragColor = vec4(0.0, 0.0, 0.11, uAlpha);
    }
  `,
});

const overlayPlane = new THREE.Mesh(overlayGeometry, overlayMaterial);
overlayPlane.rotation.y = -Math.PI * 0.5;
